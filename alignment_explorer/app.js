...
const DATA_VERSION = "20260719-sentence-steering-hf2073-11state";
  ...
  const groups = data.taxonomy.groups.map((group, index) => {
    ...
    return {
      ...
      model_counts: Object.fromEntries(
        models.map((model) => [
          model.id,
          group.examples[model.id]?.count || 0,
        ]),
      ),
    ...
    traces.map((trace, index) => `
      <option value="${index}">
        ${trace.pmcid.slice(0, 8)} · ${
          formatNumber(trace.counts[model.id])
        } sentences
      </option>
    `).join(""),
  ]));
...
function renderMetrics() {
  $("#dataset-note").textContent =
    `${state.data.summary.groups} groups`;
...
async function loadTraceCase(index) {
  ...
  if (!trace.loadPromise) {
    trace.loadPromise = fetchJson(trace.file)
      .then((data) => {
        trace.models = {};
...
function setTraceIndex(index) {
  const nextIndex = Math.max(
    0,
    Math.min(state.data.traces.length - 1, index),
  );
  if (nextIndex === state.traceIndex) {
    return;
  }
...
function traceRowStyle(row, groupByIndex, dimensionByIndex) {
  ...
  const color = group ? group.color : groupColor(row.group);
...
async function renderTraces(group) {
  ...
  $("#trace-meta").textContent =
    ...
    } sentences for ${state.data.models.find(
  ...
  $("#case-select").value = String(state.traceIndex);
...
function progressChartHtml(rows, groupByIndex, dimensionByIndex) {
  ...
  const xIndex = (index) => left + (index / maxIndex) * plotWidth;
  const x = (point) => xIndex(point.index);
  ...
  const sampled = points.length < rows.length
    ? `<span>${formatNumber(points.length)} charted sentences</span>`
    : "";
  const trace = state.data.traces[state.traceIndex];
  const xTicks = [...new Set(
    [0, 0.25, 0.5, 0.75, 1].map((value) =>
      Math.round(value * maxIndex)
    ),
  )];
  return `
    <div class="progress-toolbar">
      <button
        class="plot-nav-button"
        type="button"
        data-case-step="-1"
        ${state.traceIndex === 0 ? "disabled" : ""}
      >
        Previous
      </button>
      <label class="case-select-control">
        <span>
          Case ${formatNumber(state.traceIndex + 1)} of ${
            formatNumber(state.data.traces.length)
          } · ${trace.pmcid.slice(0, 8)}
        </span>
        <select id="case-select">
          ${state.data.trace_options[state.traceModel] || ""}
        </select>
      </label>
      <button
        class="plot-nav-button"
        type="button"
        data-case-step="1"
        ${state.traceIndex === state.data.traces.length - 1 ? "disabled" : ""}
      >
        Next
      </button>
    </div>
    <svg class="progress-chart" viewBox="0 0 ${width} ${height}" role="img">
      ...
      <g class="progress-x-axis">
        ${xTicks.map((index) => `
          <line
            x1="${xIndex(index).toFixed(1)}"
            y1="${height - bottom}"
            x2="${xIndex(index).toFixed(1)}"
            y2="${height - bottom + 5}"
          ></line>
          <text
            x="${xIndex(index).toFixed(1)}"
            y="${height - bottom + 19}"
          >#${rows[index] ? rows[index].section : index}</text>
        `).join("")}
      </g>
      ${points.map((point) => {
      ...
      <text class="axis-label" x="${width / 2}" y="${height - 6}">sentence #</text>
    </svg>
...
function renderDetail() {
  ...
  $("#detail-step").textContent = `Step ${String(group.step).padStart(2, "0")}`;
  $("#detail-title").textContent = group.title;
  $("#detail-count").textContent = `${formatNumber(group.count)} sentences`;
...
function bindEvents() {
  ...
  $("#panel-traces").addEventListener("change", (event) => {
    if (event.target.id === "case-select") {
      setTraceIndex(Number(event.target.value));
    }
  });
  $("#trace-progress-plot").addEventListener("click", (event) => {
    const caseButton = event.target.closest("[data-case-step]");
    if (caseButton) {
      setTraceIndex(state.traceIndex + Number(caseButton.dataset.caseStep));
      return;
    }
